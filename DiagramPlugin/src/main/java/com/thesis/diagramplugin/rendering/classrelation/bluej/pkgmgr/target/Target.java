...
public abstract class Target extends Vertex
    ...
    protected static int calculateWidth(String name) {
        int width = 0;
        if (name != null) {
            width = (int) new Font("SansSerif-bold", Font.PLAIN, 12).getStringBounds(name, FRC).getWidth();
        }
    ...
    public void save(Properties props, String prefix) {
        ...
        if (this instanceof IExpandable && ((IExpandable) this).isExpanded()) {
            props.put(prefix + ".width", String.valueOf(((IExpandable) this).getRegWidth()));
            props.put(prefix + ".height", String.valueOf(((IExpandable) this).getRegHeight()));
        } else {
